...
public class TaskTests {
    ...
    public void testTask05_02() {
        ...
        assertEquals("\u0000\u0000\u0000\u0000\u0000hello world", resultLines[2]);
        assertEquals("a hidden message", resultLines[3]);
        printResult("Task05-2", resultLines);
    }

    @Test
    public void testTask06() {
        System.out.println("Running Task06:");
        byte[] result = Tasks.TASK_06.handle(extractArgs("-in .\\tasks\\task06\\ciphertext.txt"));
        String[] resultLines = extractResultLines(result);
        assertEquals(1, resultLines.length);
        assertEquals("The secret message is: When using a stream cipher, never use the key more than once", resultLines[0]);
        printResult("Task06", resultLines);
    }
    ...
    private static String[] extractResultLines(byte[] result) {
        String full = new String(result);
        return full.split("\n");
    }
